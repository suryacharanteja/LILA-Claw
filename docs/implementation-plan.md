...
Version: 1.1  
Status: Approved plan; full-scope tracking clarified — see approval record  
...
All BRD scope is retained across the following milestones as well as P1 M0–M8. The later entries are tracked delivery milestones, not claims that their detailed addenda or features are already complete. IDs S-* denote staged modules whose release assignment remains as specified in the approved baselines; numbering does not place them after P4.

| Milestone | Scope covered | Entry / exit |
|---|---|---|
| P2-M1 | Telegram pairing, remote instructions/status and action-bound approvals (BR-16) | Approved P2 addenda and P1 command foundation; owner identity, deduplication, callback/expiry and delivery tests, then owner UAT |
| P3-M1 | Recurring workflows, timezone, cancellation, missed runs and overlap control (BR-17) | Approved P3 addenda; durable trigger, DST, restart and cancellation tests, then owner UAT |
| P4-M1 | Additional channel and advanced recurring workflows as selected (BR-18) | Use-based prioritization and approved addenda; channel parity/authorization and recurring-workflow tests, then owner UAT |
| S-NET | Professional discovery, connection requests, outreach, follow-ups, relationship context and batch planning (BR-05) | Approved module addendum/release assignment; recipient scope, duplicate prevention, contextual content and verified outcomes, then owner UAT |
| S-CONTENT | Posts in the owner's voice and authorized publishing (BR-06) | Approved module addendum/release assignment; grounded drafts, review/version invalidation, publishing authority and outcomes, then owner UAT |
| S-ENGAGE | Relevant reactions, comments and professional milestone catch-up (BR-07) | Approved module addendum/release assignment; context relevance, permissions and recorded outcomes, then owner UAT |
| S-COMPANY | Company following, company/page workflows, community discovery and outreach (BR-08) | Exact supported actions/release assignment approved; entity/account scope, handoff and outcome tests, then owner UAT |
| S-WITHDRAW-I | Pending connection-invitation withdrawal (BR-19) | With networking under ADR-029; consequence preview/standing policy and confirmed/uncertain outcome tests |
| S-WITHDRAW-A | Application withdrawal where platform-supported (BR-19) | After invitation withdrawal and validated applicable flow; explicit support boundary, authority and outcome tests; manual route when unavailable |
| S-FORMS | Additional known LinkedIn form coverage beyond initial search/application catalog (BR-09) | Validated catalog expansion and release review; field meaning, facts, permissions and outcome qualification |

Cross-cutting BR-01/02/10–15/20–25 are delivered through P1 M0–M8 and regression-tested for every later module. Optional local inference and Native Messaging setup help remain optional/future considerations, not silently committed release milestones. Platform-dependent actions retain the BRD feasibility boundaries.
...
The sponsor approved the plan conditional on in-scope feature coverage; see the [approval and scope clarification](approval-records/implementation-plan-approval.md). Version 1.1 makes retained later-scope milestone tracking explicit without changing the approved phase sequence or inventing release assignments. Begin M0 foundation, then dependency-ordered implementation with the existing requirement-change, UAT and release gates. No repeat approval is needed for the already approved specifications. Future-phase detailed addenda remain required.

## Milestone verification and owner UAT

Recommended execution loop: implement the milestone → run technical verification → provide a working handoff with manual UAT checklist → owner tests and reports findings → fix and regress affected behavior → owner accepts the user-facing increment. Run an integrated UAT pass again at M8 before release.

Technical verification is broader than a sanity check: relevant unit/contract tests, integration and smoke tests, regression of affected behavior, and security/recovery tests where applicable. A smoke/sanity pass demonstrates basic operation, not complete acceptance. Do not hand the owner an untested build as a substitute for developer verification.

Every handoff contains exact build/version and milestone IDs; what changed; requirements covered; how to launch/reset; numbered manual cases with expected results; automated results and not-run tests; known issues/limits; and a feedback template. The owner reports Pass/Fail with observed behavior and optional evidence; failed cases become tracked defects linked to the same requirements. No real application/message/withdrawal test is implied without explicit action authorization.

M0 and backend-only portions of M1/M2 receive engineering evidence and an assisted demonstration, not artificial business UAT for screens that do not exist. Manual UAT starts when the relevant behavior is usable and continues at meaningful increments. Such technical milestones may be Verified internally while later integrated/user acceptance remains pending.

Use separate fields for engineering status and UAT status. Engineering Verified does not equal UAT Passed or Release Approved. Unrelated authorized work can proceed while feedback is pending if it does not depend on the disputed behavior; do not label the affected feature accepted or release it early. Requirements changes go through impact review; defect fixes within approved behavior do not require repeating design approval.

## Current execution record — 9 September 2026

M4 W04.1 has begun on its approved M2 dependency while M3 qualification remains pending. Initial browser command validation and its scope/fingerprint tests are recorded in [M4 evidence](implementation-evidence/M4/README.md). No browser dispatcher or live execution is enabled by this increment; M3 is not marked complete.

The owner directed completion of Phase 1 before Phase 2, then Phase 3 and later phases; see [phase sequencing](phase-sequencing.md). M0 is engineering Verified with [evidence](implementation-evidence/M0/README.md). Earlier planning-time statements that implementation had not begun are historical. Business UAT and release acceptance remain pending.

Current M1 update: runtime/authentication/storage implementation and automated checks are available in [M1 evidence](implementation-evidence/M1/README.md). M1 remains In review only for temporary certificate cleanup; both technical decisions are resolved and Chrome qualification passed. M2 is engineering Verified against doubles; results, traceability and later integration obligations are recorded in [M2 evidence](implementation-evidence/M2/README.md). M3 is actively In progress: checkpoint/graph, provider accounting, grounding and real worker extraction have local verification; authenticated task assignment and the HTTP graph adapter now run in the separate worker with fixture browser/provider coverage; M4 browser integration, token-bound qualification and factual/provider acceptance remain open. See [M3 evidence](implementation-evidence/M3/README.md). Phase 1 remains the only active product phase.
