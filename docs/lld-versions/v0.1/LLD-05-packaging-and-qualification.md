...
Version: 0.1  
Status: Proposed; G-05 installer/signing gate open  
Parent: [LLD review package](../LLD-LILA-Claw.md)

## Distribution layout

Proposed per-user application root under LocalAppData/LILAClaw: versions/{app_version}/ for immutable runtime/assets; data/ for active encrypted stores; staging/ for updates/restores; runtime/ for nonsecret instance metadata; logs/ for sanitized diagnostics. Exact install root requires installer validation. User-selected backup/export destinations are outside version directories and never recursively deleted by uninstall without explicit selection.
...
G-05 must select installer/runtime packaging, signing algorithms/trust bootstrap and key rotation, release hosting and secure artifact retrieval. No publisher certificate or trusted release source is invented here. The selected implementation must meet this sequence without bypassing owner update confirmation.
...
Close G-01/G-02 shared contracts before their dependent code; pin G-03 interfaces and provider, then implement bounded execution; qualify G-04 adapters before enabling their actions; close G-05 before distributing packages. Review affected LLD revisions before implementation under the agreed process. Do not enable incomplete adapters or plaintext fallbacks to make end-to-end demos appear successful.
