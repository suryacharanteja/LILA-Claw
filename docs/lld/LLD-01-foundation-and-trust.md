...
Read with [selected trust protocols](LLD-06-selected-protocols-and-dependencies.md) and [typed contracts](LLD-07-contracts-and-invariants.md); those chapters supply the final authentication choices and exact route bodies.
...
## Trust protocol requirements — implemented by LLD-06
...
LLD-06 selects loopback TLS and constrained per-user certificate setup, fragment bootstrap, secure cookie/CSRF sessions, signed nonce extension pairing and inherited worker-pipe credentials. Implement that protocol; the envelope alone is not authentication. No secret in query strings, command-line arguments, durable checkpoints or logs.
