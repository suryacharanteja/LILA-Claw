...
UUID primary keys unless noted; enforce foreign keys and NOT NULL for ownership/state/version fields. Coordinator serializes writes. Provider monetary amounts are integer micro-USD under LLD-06, with rounding at display boundaries; revisions and sequences are integers. Schema migrations are sequential, checksummed and tracked in schema_migrations(version, checksum, applied_at). The authoritative executable DDL is [schema-v1.sql](contracts/schema-v1.sql); the table below summarizes entities, not literal column names.
...
The supplied DDL uses normalized joins, foreign keys, checks and a submission guard. JSON content versions additionally validate against LLD-07 schemas; they do not replace authority relationships. Enforce no two active mutating submission attempts per application using the transaction-backed uniqueness guard. Unknown job identity remains non-dispatchable until reviewed rather than manufacturing a global ID from title similarity.
...
## Encryption and portable formats

LLD-06 fixes SQLCipher 0.6.2 and connection settings, AESGCM artifact/package formats, nonce/key separation, audit canonicalization/HMAC, recovery wrapping and rotation. Implement those formats with established libraries; the format identifiers are versioned and require migration if changed.
