...
LLD-06 maps these application operations to the selected BaseCheckpointSaver sync/async methods, serializer and pending-write rules; LLD-07 supplies transport bodies and DDL. Interrupt/replay must re-enter coordinator gates; replaying a graph node cannot generate a new external submission identity for an existing logical action.
...
LLD-06 selects the exact initial model, direct Responses API, price/configuration policy, request/output bounds, credential handoff and data controls. LLD-08 defines factual/cost acceptance. No provider purchase or real personal-data evaluation is authorized by this draft.
