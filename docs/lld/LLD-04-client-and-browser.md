...
Envelope: the BrowserAction schema in protocol-v1.schema.json, including protocol_version, action_id, run_id, generation, account_id, tab_id, allowed_origin, payload_version, expires_at, tool, adapter_id, optional form_fingerprint and typed arguments. Authenticated socket identity uses LLD-06 pairing/session proofs. Reject expired commands, unsupported versions/tools, mismatched context and arbitrary URLs outside scope.
...
LLD-06 fixes semantic selector strategy, packaged function/CDP boundaries, upload handling and positive/negative confirmation rules. Implementers capture actual reviewed fixtures and qualify each enabled variant during M4 using those rules; the design does not fabricate live DOM evidence. Unsupported external employer pages remain handoff-only.
